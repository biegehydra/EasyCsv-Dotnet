...
public class StringSplitColumnStrategy : IFullCsvProcessor
{
    ...
    public StringSplitColumnStrategy(string columnToSplit, string[] newColumnNames, string delimiter, bool removeSplitColumn, StringSplitOptions stringSplitOptions = StringSplitOptions.RemoveEmptyEntries, Func<double, Task>? onProgress = null)
    {
        ...
        _splitColumnStrategy = new SplitColumnStrategy(columnToSplit, newColumnNames, removeSplitColumn,
            x => x?.ToString()?.Split([delimiter], stringSplitOptions)?.Cast<object?>().ToArray(), onProgress);
    }
